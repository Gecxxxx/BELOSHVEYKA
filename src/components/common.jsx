...
export function PageHero({label,title,description,image=null,imageAlt='',action=true}){return <section className={'page-hero'+(!image?' text-only':'')}><div className="page-hero-copy"><a className="breadcrumb" href="/">Главная /</a><Label>{label}</Label><h1>{title}</h1><p>{description}</p>{action&&<Button dark/>}</div>{image&&<figure className={'page-hero-visual'+(image.startsWith('concept/')?' scene':'')}><Photo name={image} alt={imageAlt} eager/><figcaption>{imageCredit(image)}</figcaption></figure>}</section>}
